        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
          <div id="carouselCaptions" class="carousel slide relative" data-bs-ride="carousel">
            <div class="carousel-indicators absolute right-0 bottom-0 left-0 flex justify-center p-0 mb-4">
              <button v-for="(img, i) in product?.attributes?.image.data ?? []" type="button"
                data-bs-target="#carouselCaptions" :data-bs-slide-to="i" :class="[i == 0 ? 'active' : null]"
                :aria-current="i == 0 ? 'true' : 'false'" :aria-label="`Slide ${i + 1}`"></button>
            </div>
            <div class="carousel-inner relative w-full overflow-hidden">
              <div v-for="(img, i) in product?.attributes?.image.data ?? []"
                :class="['carousel-item float-left w-full', i == 0 ? 'active' : null]">
                <img :src="img?.attributes?.url" class="block object-cover md:w-[35rem] md:h-[35rem] rounded"
                  alt="..." />
              </div>
            </div>
            <button
          ...
          <div class="px-8 flex flex-col">
            ...
            <p class="text-xl font-bold text-yellow-500 mt-4">Rp. {{
              Number(product?.attributes?.price).toLocaleString('id')
            }}</p>
            <p class="mt-4">{{ product?.attributes?.description }}</p>
            <a class="mt-6 rounded-xl px-5 py-4 bg-green-300 text-xl text-center" target="_blank"
              :href="`https://wa.me/6289513080754?text=${text}`">Buy Via Whatsapp</a>
          </div>
        </div>
      </div>
    </div>
