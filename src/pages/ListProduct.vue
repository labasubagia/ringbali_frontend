...
const filterForm = () => {
  router.push({
    path: route.path,
    query: { key: filterKey.value, categoryId: filterCategoryId.value },
  });
};
...
<template>
  <MainLayout>
    <div class="container mx-auto mt-12">
      <header class="my-4 text-3xl font-bold flex items-center justify-center">
        Produk
      </header>

      <form
        class="flex flex-col md:flex-row p-4 justify-center"
        @submit.prevent="filterForm"
      >
        <input
          class="border-2 p-1 px-2 rounded mt-2 border-gray-300 md:mr-2"
          type="text"
          v-model="filterKey"
        ...
        <select
          class="bg-gray-100 p-1 px-2 mt-2 border-gray-300 border-2 rounded md:mr-2"
